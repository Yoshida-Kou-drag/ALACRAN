...
#COMポートを開く
b3m = serial.Serial('COM4', baudrate=1500000, parity=serial.PARITY_NONE, timeout=0.5)

#B3M_Read_CMD(servo_id, Data_size, Address)
#ID番号を読み込む(ID:0,1byte読み込み,アドレス：0x00(ID))
bl, reData = B3M_Read_CMD(0, 1, 0x00)
print(bl, reData)

#現在位置を読み込む(ID:0,2byte読み込み,アドレス：0x2C(現在位置))
bl, reData = B3M_Read_CMD(0, 2, 0x2C)
print(bl, reData)

#通信速度を読み込む(ID:0,4byte読み込み,アドレス：0x01(通信速度))
bl, reData = B3M_Read_CMD(0, 4, 0x01)
print(bl, reData)
