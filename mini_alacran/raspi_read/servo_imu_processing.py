...
def servo_moving(imu_data,calib_data,left_deg_input,right_deg_input):
    ...
    for i in range(1):
        ...
        left_deg_input.value, right_deg_input.value=5, 0
        ...
        left_deg_input.value, right_deg_input.value=0, 0
        left_deg, right_deg= int(left_deg_input.value)+15, -int(right_deg_input.value)+21
        pi.hardware_PWM(gpio_pin0,50,int(deg_to_duty(right_deg)*10000))
        pi.hardware_PWM(gpio_pin1,50,int(deg_to_duty(left_deg)*10000))
        time.sleep(2)

        left_deg_input.value, right_deg_input.value=0, 5
        left_deg, right_deg= int(left_deg_input.value)+15, -int(right_deg_input.value)+21
        pi.hardware_PWM(gpio_pin0,50,int(deg_to_duty(right_deg)*10000))
        pi.hardware_PWM(gpio_pin1,50,int(deg_to_duty(left_deg)*10000))
        time.sleep(2)
...
if __name__ == "__main__":
    ...
    left_deg_input = multiprocessing.Value('i',0)
    right_deg_input = multiprocessing.Value('i',0)
    # filter_imu = multiprocessing.Array('d', 3)

    try :
        p1 = multiprocessing.Process(name="p1", target=read_imu, args=(imu_data,),daemon=False)
        p2 = multiprocessing.Process(name="p1", target=write_csv, args=(imu_data,calib_data,left_deg_input,right_deg_input),daemon=False)
